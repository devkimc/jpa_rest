...
public interface RestRepository extends JpaRepository<Rest, String> {
    ...
    @Query("SELECT " +
            "NEW jparest.practice.rest.dto.GetFavRestListResponse(r.id, r.restName, r.latitude, r.longitude) " +
            "FROM Rest r JOIN r.groupRests gr WHERE gr.group.id = :groupId")
    List<GetFavRestListResponse> findAllByGroupId(@Param("groupId") Long groupId);
}
